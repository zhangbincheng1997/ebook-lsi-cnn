...
from keras.layers import Input, Dense, Activation, Dropout, Flatten, Reshape, concatenate, multiply
...
def lstm():
    input_q = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='float64')
    q = embeding()(input_q)
    q = Bidirectional(LSTM(QA_EMBED_SIZE, return_sequences=True, dropout=DROPOUT_RATE, recurrent_dropout=DROPOUT_RATE), merge_mode="sum")(q)
    ...
    input_a = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='float64')
    a = embeding()(input_a)
    a = Bidirectional(LSTM(QA_EMBED_SIZE, return_sequences=True, dropout=DROPOUT_RATE, recurrent_dropout=DROPOUT_RATE), merge_mode="sum")(a)
